...
print(ggplot(datalistTopicYearDf, aes(x = year %m-% months(12) , y = relativePercentage, fill = topics, group = topics, colour = topics)) +scale_y_continuous(name = "Percentage (%)")+ geom_line()+geom_point()+facet_wrap(~ factor(topics, levels = c(1,2,3,4,5)))+ theme(legend.position = "none") + xlab("year"))
...
correspondence <- row.names(table(sentimentEmotionsScoresDf$correspondence))
plotSentimentPiePercentage <- function(Sentiment_Emotion,reference,topic,correspondence){
label = c()
  for(t in reference){
...
plotSentimentPiePercentage(emotions,originalTopics,TRUE,FALSE)
plotSentimentPiePercentage(sentiment,originalTopics,TRUE,FALSE)
plotSentimentPiePercentage(emotions,correspondence,FALSE,TRUE)
plotSentimentPiePercentage(sentiment,correspondence,FALSE,TRUE)
...
plotSentimentAanalysisOverTime <- function(Sentiment_Emotion, facet_rows, facet_col, wrap){
  ...
  if(wrap == TRUE){ 
  print(ggplot(datalist, aes(x = year %m-% months(12) , y = relativePercentage, fill = Sentiment_Emotion, group = Sentiment_Emotion, colour = Sentiment_Emotion)) +scale_y_continuous(name = "Percentage (%)")+geom_line()+geom_point()+facet_wrap(~ Sentiment_Emotion, nrow = facet_rows, ncol = facet_col)+xlab("year")+ theme(legend.position = "none",axis.text.x = element_text(angle = 0, vjust = 1, hjust=0.7)))
  }
  else{
    print(ggplot(datalist, aes(x = year %m-% months(12) , y = relativePercentage, fill = Sentiment_Emotion, group = Sentiment_Emotion, colour = Sentiment_Emotion)) +scale_y_continuous(name = "Percentage (%)")+geom_line()+geom_point()+xlab("year")+ theme(axis.text.x = element_text(angle = 0, vjust = 1, hjust=0.5)))
  }
}
