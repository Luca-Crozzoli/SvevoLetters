...
plotSentimentAanalysisOverCorrespondence <- function(SentimentEmotionVector){
for(c in correspondence){
  sentimentdf_partial <- sentimentEmotionsScoresDf[sentimentEmotionsScoresDf$correspondence == c,] 
  WordsForSentiment = c()
  ...
  #we create the appropriate data frame to plot the percentage of each topic of each emotion
  SentimentEmotiondf <- data.frame(relativePercentage, SentimentEmotionVector)
  ...
  coord_polar(theta = "y") +
  theme_void()+
  ggtitle(paste0("Correspondence ", c)))
}
}

plotSentimentAanalysisOverCorrespondence(emotions) 
plotSentimentAanalysisOverCorrespondence(sentiment)
```
```{r}
plotSentimentPiePercentage <- function(SentimentEmotionVector,reference,topic,correspondence){
label = c()
  for(t in reference){
