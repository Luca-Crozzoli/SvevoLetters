...
##Step 1 Load Data
```{r}
rawData <- read.csv(file = "C:/Users/User/Documents/UNITS/Magistrale/Primo Anno/1°semestre/Machine Learning/SvevoLetters/carteggiosvevoSHORT80PULITO.csv", sep = ";", header = TRUE)
head(rawData)
rawData$n <- rawData$n +1
...
##create the corpus composed only by the text data removing non ascii characters
```{r}
myCorpus <-VCorpus(VectorSource(rawData$text))
...
for(k in kvalues){
  ...
  coherence <- c(coherence, m)
}
data_graph_k_C <- data.frame(coherence,kvalues)
```

##visualizaation of coherance with teh number of topics
```{r}
data_graph_k_C%>%mutate(max_k = which(coherence == max(coherence)) * 10) %>% 
   ggplot(aes(kvalues, coherence)) +
...
letters_word_topic <- GetTopTerms(lda_news$phi,30)%>% as.data.frame()%>%set_names(paste("topic",1:30))
...
##grphical way to reppresents topic with related words
```{r}
letters_word_topic %>% 
...
```{r}
letters_doc_topic <- as.data.frame(lda_news$theta)%>%rownames_to_column("id")
...
```{r}
letters_doc_topic2 <- lda_news$theta %>% 
   as.data.frame() 

toptopics <- as.data.frame(cbind(id=row.names(letters_doc_topic2), topic= apply(letters_doc_topic2,1,function(x)names(letters_doc_topic2)[which(x==max(x))])))
...
df_topic_date <-table_topic_date%>%pivot_longer(c(t_1, t_2, t_3), names_to = "topic", values_to = "theta") %>% 
