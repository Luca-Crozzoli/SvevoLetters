...
#create_tfidf <- function(unionunibitridtm){
##create the tfidf
#unionunibitridmTFIDF <- weightTfIdf(unionunibitridtm)
#terms <- findFreqTerms(unionunibitridmTFIDF, lowfreq = 0.02)
#unionunibitridmTFIDF <- unionunibitridmTFIDF[,terms]

#m <- as.matrix(unionunibitridmTFIDF)
#v <- sort(colSums(m),decreasing=TRUE)
#d <- data.frame(freq=v,word = names(v))
#head(d, 10)
#set.seed(1234)
...
#unionunibitridmTFIDF <- weightTf(unionunibitridmTFIDF)
#return(unionunibitridmTFIDF)
#}

dtmtf <- createdtmtf(FALSE)
#dtmtfidf <- create_tfidf(dtmtf)

dtmtfpruned <- createdtmtf(TRUE)
#dtmtfidfpruned <- create_tfidf(dtmtfpruned)
```
