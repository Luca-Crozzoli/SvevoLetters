...
ggplot(doctopicclassification,mapping = aes(x= date, fill = topic, group=topic, colour = topic))+ geom_freqpoly() + facet_wrap(facets = vars(topic)) + theme(legend.position = "none")

```
##then with the doctopicclassifcation we add the corpus categories
```{r}
#count the pairs of sender and receivers and the number of letters exchanged puttin them in descending order
doctopicclassification["categorie_sender_receiver"] <- rawData$corpus
#-----------------------------------------------------------------------------
correspondances <-table(doctopicclassification$categorie_sender_receiver) 
correspondances_ordered <- correspondances[order(correspondances, decreasing = TRUE)]
correspondances.df <- as.data.frame(correspondances_ordered)
#------------------------------------------------------------------------------
ggplot(data=doctopicclassification, aes(x= categorie_sender_receiver, fill = topic))+geom_bar()+ coord_flip()
...
library(lessR)
#for( i in 1:9){
  doctopicclassification_partial <- doctopicclassification[which(doctopicclassification$topic == 1),]
  table <- table(doctopicclassification_partial$categorie_sender_receiver)
  pie(table)
#}
