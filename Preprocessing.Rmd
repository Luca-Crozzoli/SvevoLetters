...
install.packages("svglite") #to save plots in svg
...
##Step 1 Load Data
```{r}
rawData <- read.csv(file = "C:/Users/User/Documents/UNITS/Magistrale/Primo Anno/1°semestre/Machine Learning/SvevoLetters/VABENE.csv", sep = ";", header = TRUE, encoding = "Latin-1")
head(rawData)
...
x <- as.data.frame(x)
x <- x[!x$upos == "PUNCT",] #punctuation removed because we will remove it using tm library
...
barchart(key ~ freq, data = stats, col = "cadetblue", 
         ...
         xlab = "Freq")
taggedTerms <- names(table(x$upos))

printFreq <- function(tag){
  stats <- subset(x, upos %in% tag) 
stats <- txt_freq(stats$lemma)
stats$key <- factor(stats$key, levels = rev(stats$key))
print(barchart(key ~ freq, data = head(stats, 30), col = "cadetblue", 
         main = paste0("Most occourign",tag), xlab = "Freq"))
  
}

for (tag in taggedTerms){
  printFreq(tag)
}
#noun
...
verbstoremovedf <- subset(stats, stats$freq >= 1000)
verbstoremove <- as.character(verbstoremovedf[,"key"])
...
myStopwords2 <- c(pronounstoremove,adptoremove,dettoremove,c("italo","svevo","ettore","schmitz","caro","perca","xe","siora","mulo","piu","ora","oggi", verbstoremove))
chunk <- 500
n <- length(myStopwords2)
r <- rep(1:ceiling(n/chunk),each=chunk)[1:n]
d <- split(myStopwords2,r)
...
##visualizaation of coherance with the number of topics
```{r}
plot <- function(data_graph_k_C_s){
data_graph_k_C_s <- data_graph_k_C_s %>%
  rowwise()%>%
  mutate(DiffSC = similarity - coherence)

ideal_5_keys <- head(data_graph_k_C_s[order(data_graph_k_C_s$DiffSC),],2)
ideal_k <- as.numeric(ideal_5_keys[ideal_5_keys$DiffSC == min(ideal_5_keys$DiffSC), "kvalues"])
#ideal_k <- as.numeric(ideal_5_keys[ideal_5_keys$kvalues == min(ideal_5_keys$kvalues), "kvalues"])
print(as.numeric(data_graph_k_C_s[data_graph_k_C_s$kvalues == ideal_k,"DiffSC"]))
...
originalTopics <- seq(1,Ntopicpruned, by=1)
kTopic <- length(originalTopics)
...
```{r}
lda$phi%>% rowSums()
letters_word_topic <- GetTopTerms(lda$phi,30)%>% as.data.frame()%>%set_names(paste("topic",1:kTopic))
...
library(scales)
show_col(hue_pal()(Ntopicpruned))

colors = c("#F8766D","#D39200","#93AA00","#00BA38","#00C19F","#00B9E3","#619CFF","#DB72FB","#FF61C3")
printCloud <- function(column, color){
  letters_word_topic_partial <- letters_word_topic[,column]%>%as.data.frame()%>%
    rownames_to_column("id") %>%
   mutate(id = as.numeric(id)) %>% 
   pivot_longer(-id, names_to = "topic", values_to = "term")
  
  print(ggplot(letters_word_topic_partial, aes(label = term, color = color , size = rev(id))) +
   geom_text_wordcloud(seed = 123) +
   geom_text_wordcloud_area(rm_outside = TRUE, area_corr_power = 1)+
   scale_color_manual(values = c(color))+
     labs(title = paste0("Topic ",column))+
     theme_minimal()+
     theme(plot.title = element_text(hjust = 0.5, vjust = 1) ))
} 

vector()
columns <- ncol(letters_word_topic)
for (i in 1:columns){
  printCloud(i,colors[i])
  ggsave(filename = paste0("Cloud_for_topic_",i,".svg"))
}
...
docThetaTopic <- lda$theta %>% 
   as.data.frame()
colnames(docThetaTopic) <- originalTopics#c(1,2,3,4,5)
rownames(docThetaTopic) <- row.names(rawData)
#docThetaTopic$id <- row.names(rawData)

topicRating <- gather(docThetaTopic, col,val)
#result <-group_by(topicRating, id) %>% arrange(id,desc(val))%>% summarise(top1 = first(col),top2 = nth(col,2),top3 = nth(col,3))
...
#######################################first approach to reveal th emost important topic##
docThetaTopic2 <- docThetaTopic
docThetaTopic2$id <- row.names(rawData)
df2 <- gather(docThetaTopic2,topic,val,-id)   
res <- group_by(df2,id) %>% arrange(id,desc(val)) %>% summarise(top1 = first(topic),top2 = nth(topic,2),top3 = nth(topic,3), top4 = nth(topic,4))
res[order(res$id),]

top1 <-as.data.frame( table(res$top1))
top2 <- as.data.frame(table(res$top2))
top3 <-as.data.frame(table(res$top3))
top4 <-as.data.frame (table(res$top4))
#####################second approach to find most relevant topics
topicProprtions <- colSums(docThetaTopic) / nrow(docThetaTopic)
sort(topicProprtions, decreasing = TRUE)
...
datalistTopicYearDf <- data.frame(year = integer(),topics =integer(),numberTopicsYear= integer(), relativePercentage = integer() )

for(year in yearsvector){
  docTopicClassification_year <- docTopicClassificationDf[docTopicClassificationDf$year == year,]
    
    #count the number of letters related to each topic from a specific year
    ...
    missingTopicForYear <- factor(setdiff(originalTopics,topics))
    iterations <- length(missingTopicForYear)
    
    #if the number of missing topics per year is different form zero we count 0
    # for all the missing topics
    if(iterations != 0){
    
      for(i in 1:iterations){
        row <- data.frame(year = year, topics = missingTopicForYear[i], numberTopicsYear = 0)
        letterTopicYearDf <- rbind(letterTopicYearDf,row)
      }
    }
    
    totalLettersYear <- sum(letterTopicYearDf$numberTopicsYear)
    
    letterTopicYearDf <- letterTopicYearDf%>% mutate(relativePercentage = numberTopicsYear/totalLettersYear*100)
...
#print(ggplot(datalistTopicYearDf, aes(x = year, y = numberTopicsYear, fill = topics, group = #topics, colour = topics)) +scale_y_continuous(name = "count")+ #geom_line()+geom_point()+facet_wrap(~ factor(topics, levels = originalTopics))+ #theme(legend.position = "none") + xlab("year"))


print(ggplot(datalistTopicYearDf, aes(x = year, y = relativePercentage, fill = topics, group = topics, colour = topics)) +scale_y_continuous(name = "Percentage (%)")+ geom_line()+geom_point()+facet_wrap(~ factor(topics, levels = originalTopics))+scale_color_manual(values = c("1"="#F8766D","2" ="#D39200","3"= "#93AA00","4"="#00BA38","5"="#00C19F","6"="#00B9E3","7"="#619CFF","8"="#DB72FB","9"="#FF61C3"))+ theme(legend.position = "none",axis.text.x = element_text(angle = 0, vjust = 1, hjust=0.7)) + xlab("year"))

ggsave("TopicProprtionOverTime.svg")
...
library(randomcoloR)
set.seed(2)
pal <- rainbow(length(unique(docTopicClassificationDf$correspondence)))
pal = setNames(pal, unique(docTopicClassificationDf$correspondence))

for(i in originalTopics){
...
print(ggplot(correspondencePercentageTopicDf, aes(x = "" , y = perc, fill = correspondence)) +
  ...
  ggsave(filename = paste0("correpondence_for_topic_",i,".svg"))
}
```
...
sentimentEmotionsScoresDf <- cbind(docTopicClassificationDf, lettersVector,sentimentEmotionsScores)

#barplot for sentiment
overallSentimentEmotion <- subset(sentimentEmotionsScores, select = -c(9,10))
svg("overall_sentiment.svg")
barplot(colSums(overallSentimentEmotion), col = rainbow(8), ylab = "Count", main = "Overall sentiment analysis", las = 2)
dev.off()

```

#plot the graph to see the sentiment analysis for each topic and each correspondence
```{r}
emotions<-c("anger","anticipation","disgust","fear","joy","sadness","surprise","trust")
sentiment <- c("negative","positive")
correspondenceTable <- table(sentimentEmotionsScoresDf$correspondence)
#we want to see the most frequent correspondences of svevo
orderedCorrespondence <- correspondenceTable[order(correspondenceTable, decreasing = TRUE)]
print(orderedCorrespondence)

correspondence <- row.names(correspondenceTable)
plotSentimentPiePercentage <- function(Sentiment_Emotion,reference,topic,correspondence, filename){
label = c()
  for(t in reference){
    ...
    if(correspondence == TRUE){
     sentimentdf_partial <- sentimentEmotionsScoresDf[sentimentEmotionsScoresDf$correspondence == t,]  
    label <- c("Correspondence ")
    }
    
    WordsForSentiment = c()
    
    #count the number of words of a particular sentimentemotion from all the letters of a specific topic
    for(e in Sentiment_Emotion ){
      
      value <- sum(sentimentdf_partial[,e])
      WordsForSentiment <- c(WordsForSentiment, value)
    }
    
    names(WordsForSentiment)  <- Sentiment_Emotion
   
    #we calculate the total number of words from teh different sentiments of a specific topic
    totalSentimentWordTopic <- sum(WordsForSentiment)
    
    relativePercentage <- c()
    
    #we calculate the percentage of words related to a sentiment in each topic
    for(e in Sentiment_Emotion){
    ...
    #we create the appropriate data frame to plot the percentage ofr each topic of each emotion
    SentimentEmotiondf <- data.frame(relativePercentage, Sentiment_Emotion)
    
    SentimentEmotiondf<- SentimentEmotiondf %>% mutate(labels = percent(relativePercentage, scale = 1, accuracy = 0.01))
    
    print(ggplot(SentimentEmotiondf, aes(x = "" , y = relativePercentage, fill = Sentiment_Emotion)) +
    geom_col(width = 1, color = 1) +
    geom_text(aes(label = labels), position = position_stack(vjust = 0.5))+
    coord_polar(theta = "y") +
    theme_void()+
    ggtitle(paste0(label, t)))
    
    ggsave(filename = paste0(filename,label,t,".svg"))
  }
}
plotSentimentPiePercentage(emotions,originalTopics,TRUE,FALSE,"Pie_emotions")
plotSentimentPiePercentage(sentiment,originalTopics,TRUE,FALSE,"Pie_sentiment")
plotSentimentPiePercentage(emotions,correspondence,FALSE,TRUE,"Pie_emotions")
plotSentimentPiePercentage(sentiment,correspondence,FALSE,TRUE,"Pie_sentiment")
...
plotSentimentAanalysisOverTime <- function(Sentiment_Emotion, facet_rows, facet_col, emotions_facete){

  #set the data frame to collect the data for each year
  yearsvector <- rownames(table(sentimentEmotionsScoresDf$year))
  datalist <- data.frame(relativePercentage = integer(), Sentiment_Emotion = character(),year =integer())
  ...
  for (year in yearsvector){
    ...
    #for each sentiment_emotion calculate the percentage
    for(e in Sentiment_Emotion){
    ...
    SentimentEmotiondf <- data.frame(relativePercentage, Sentiment_Emotion, year)
  ...
  numberofemotions <- length(table(datalist$Sentiment_Emotion))
  
  if(numberofemotions == 8){
    
    plotEmotionvsEmotion <- function (df, colorsValues, title){    
    print(ggplot(df, aes(x = year, y = relativePercentage, fill = Sentiment_Emotion, group = Sentiment_Emotion, colour = Sentiment_Emotion))+ scale_color_manual(values = colorsValues) +scale_y_continuous(name = "Percentage (%)")+geom_line()+geom_point()+xlab("year")+ theme(axis.text.x = element_text(angle = 0, vjust = 1, hjust=0.5))+ ggtitle(title))}
    
    datalistAngerSurprise <- datalist[datalist$Sentiment_Emotion == "anger" | datalist$Sentiment_Emotion == "surprise",]
    
    plotEmotionvsEmotion(datalistAngerSurprise, c("red","green3"), "Anger-Surprise")
    ggsave(filename = paste0("Anger_Surprise.svg"))

    datalistJoySadness <- datalist[datalist$Sentiment_Emotion == "joy" | datalist$Sentiment_Emotion == "sadness",]
    
    plotEmotionvsEmotion(datalistJoySadness, c("green3","red"), "Sadness-Joy")
    ggsave(filename = paste0("Sadenss_Joy.svg"))
    
    datalistFearTrust <- datalist[datalist$Sentiment_Emotion == "fear" | datalist$Sentiment_Emotion == "trust",]
    
     plotEmotionvsEmotion(datalistFearTrust, c("red","green3"), "Fear-Trust")
     ggsave(filename = paste0("Fear_Trust.svg"))
    
    datalistDisgustAnticipation <- datalist[datalist$Sentiment_Emotion == "disgust" | datalist$Sentiment_Emotion == "anticipation",]
    
    plotEmotionvsEmotion(datalistDisgustAnticipation, c("green3","red"), "Disgust-Anticipation")
    ggsave(filename = paste0("Disgust_Anticipation.svg"))
 
  }
 
  if(emotions_facete == TRUE){ 
  print(ggplot(datalist, aes(x = year, y = relativePercentage, fill = Sentiment_Emotion, group = Sentiment_Emotion, colour = Sentiment_Emotion)) +scale_y_continuous(name = "Percentage (%)")+geom_line()+geom_point()+facet_wrap(~ Sentiment_Emotion, nrow = facet_rows, ncol = facet_col)+xlab("year")+ theme(legend.position = "none",axis.text.x = element_text(angle = 0, vjust = 1, hjust=0.7)))
ggsave(filename = paste0("AllEmotionsOverTime.svg"))
    
  }
  else{
    print(ggplot(datalist, aes(x = year, y = relativePercentage, fill = Sentiment_Emotion, group = Sentiment_Emotion, colour = Sentiment_Emotion))+scale_color_manual(values = c("red","green3")) +scale_y_continuous(name = "Percentage (%)")+geom_line()+geom_point()+xlab("year")+ theme(axis.text.x = element_text(angle = 0, vjust = 1, hjust=0.5))+ggtitle("Negative-Positive"))
ggsave(filename = paste0("PositiveNegativeOverTime.svg"))
  }
}
