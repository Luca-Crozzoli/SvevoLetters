...
##Step 1 Load Data
```{r}
rawData <- read.csv(file = "C:/Users/User/Documents/UNITS/Magistrale/Primo Anno/1°semestre/Machine Learning/SvevoLetters/carteggiosvevo3.csv", sep = ";", header = TRUE)
head(rawData)
...
```
#creation of the dictionary ofr the lemmatization and see the frequency of noun and pronouns and verbs to choose which to remove from the corpus
```{r}
...
myStopwords <- c(stopwords("it"),stopwords("en"), stopwords("fr"),stopwords("de"), pronounstoremove)
...
#unidtm <- removeSparseTerms(unidtm, 0.90)# elimino i termini che non compaiono dall'90% in su dei dicumenti

#bigramdtm
  bidtm <- DocumentTermMatrix(myCorpus, control = list(tokenize = bigramTokenizer,wordLengths=c(2,Inf) ))
  #bidtm <- removeSparseTerms(bidtm, 0.90)#elimino i termini che non compaiono dall' 90% in su dei documenti
  
#trigramdtm
  tridtm <- DocumentTermMatrix(myCorpus, control = list(tokenize = trigramTokenizer,wordLengths=c(2,Inf) ))
  #tridtm <- removeSparseTerms(tridtm, 0.90)#elimino i termini che non compaiono dall'90% in su dei documenti
...
##DTM TFIDF  
#unigramtfidf
  unidtmtfidf <- DocumentTermMatrix(myCorpus, control = list(wordLenghts = c(2,Inf)))
unidtmtfidf <- weightTfIdf(unidtmtfidf, normalize = TRUE)
#unidtmtfidf <- removeSparseTerms(unidtmtfidf, 0.90)

#bigramtfidf
  bidtmtfidf <- DocumentTermMatrix(myCorpus,control = list(tokenize = bigramTokenizer,wordLengths=c(2,Inf) ))
bidtmtfidf <- weightTfIdf(bidtmtfidf, normalize = TRUE)
#bidtmtfidf <- removeSparseTerms(bidtmtfidf, 0.90)

#trigramtfidf
  tridtmtfidf <- DocumentTermMatrix(myCorpus,control = list(tokenize = trigramTokenizer,wordLengths=c(2,Inf) ))
tridtmtfidf <- weightTfIdf(tridtmtfidf, normalize = TRUE)
#tridtmtfidf <- removeSparseTerms(tridtmtfidf, 0.90)



#inspect unidtmtfidf
m <- as.matrix(unidtmtfidf)
v <- sort(colSums(m),decreasing=TRUE)
d <- data.frame(freq=v,word = names(v))
head(d, 10)
set.seed(1234)
...
#inspect union and bigrams
unionunibitridtmtfidf <- cbind(unidtmtfidf,bidtmtfidf,tridtmtfidf)
attributes(unionunibitridtmtfidf) <- attributes(unidtmtfidf)
m <- as.matrix(unionunibitridtmtfidf)
v <- sort(colSums(m),decreasing=TRUE)
d <- data.frame(freq=v,word = names(v))
head(d, 10)
set.seed(1234)
...
```{r}
word_freq <- findFreqTerms(unionunibitridtm,lowfreq = 1, highfreq = nrow(unionunibitridtm)*0.9)
unionunibitridtm <-unionunibitridtm[,word_freq]
m <- as.matrix(unionunibitridtm)
v <- sort(colSums(m),decreasing=TRUE)
d <- data.frame(freq=v,word = names(v))
head(d, 10)
...
word_freq2 <- findFreqTerms(unionunibitridtmtfidf,lowfreq = 1, highfreq = nrow(unionunibitridtmtfidf)*0.9)
unionunibitridtmtfidf <-unionunibitridtmtfidf[,word_freq2]
m <- as.matrix(unionunibitridtmtfidf)
v <- sort(colSums(m),decreasing=TRUE)
d <- data.frame(freq=v,word = names(v))
head(d, 10)
...
dtm_lda <- Matrix::Matrix(as.matrix(unionunibitridtm), sparse = T)
coherence = c()
similarity = c()
...
unionunibitridtmtfidf <- weightTf(unionunibitridtmtfidf)
dtm_lda_tfidf <- Matrix::Matrix(as.matrix(unionunibitridtmtfidf), sparse = T)
coherence = c()
similarity = c()
kvalues = seq(2,15,by=1)
for(k in kvalues){
  set.seed(123)
  m <- FitLdaModel(dtm =dtm_lda_tfidf,
                        k = k, 
                        iterations = 500,
                        burnin = 400, 
                        calc_coherence = T
...
data_graph_k_C_s_2 <- data.frame(coherence,similarity,kvalues)
```
...
data_graph_k_C_s_2 <- data_graph_k_C_s_2 %>%
  rowwise()%>%
  mutate(DiffCS = coherence - similarity)
...
##3K TOPICS SELECTOR------------------------
#Take the best 5 potentially k values and from the 5 we consider only 1 with the less kvalues
ideal_5_keys <- head(data_graph_k_C_s_2[order(-data_graph_k_C_s_2$DiffCS),],3)
ideal_k <- as.integer(ideal_5_keys[ideal_5_keys$kvalues == min(ideal_5_keys$kvalues), "kvalues"])
...
letters_word_topic <- GetTopTerms(lda$phi,10)%>% as.data.frame()%>%set_names(paste("topic",1:9))
...
transform(doctopicclassification, topic = as.numeric(topic))
...
```
##then with the doctopicclassifcation we add the corpus categories
```{r}
doctopicclassification$categorie_sender_receiver<- rawData$corpus

doctopicclassification$receivers <- str_replace_all(doctopicclassification$receivers,"Svevo","")
doctopicclassification$receivers <- str_replace_all(doctopicclassification$receivers,"Schmitz","")


ggplot(data=doctopicclassification, aes(x= receivers, fill =topic ))+geom_bar(position=position_dodge(preserve = "single"))+ coord_flip() + scale_y_continuous(limits = c(0,250)) 

#we consider the pair sender and receiver which have
...
for( i in 1:9){
  doctopicclassification_partial <- doctopicclassification[which(doctopicclassification$topic == i),]
  table <- table(doctopicclassification_partial$receivers)
  print(table)
  tablevector <- as.vector(table)
  
  pie_labels <- paste0(" ",paste0(round(100*table/sum(table), 2), "%"))
  pie(table,labels = pie_labels, col = rainbow(length(table)), main = paste0("topic ", i))
  legend("bottomleft", legend = rownames(table), fill = rainbow(length(table)))
}


#  categories_var <- data.frame(cat=doctopicclassification_partial$categorie_sender_receiver)
#  PieChart(cat,hole =0, values = "%", data = categories_var, fill = "rainbow", main = "")

#correspondances.df[correspondances.df$topic == 1,]
```
```{r}
...
df2 <- dfpartial_topic %>% mutate(csum = rev(cumsum(rev(perc))), 
         pos = perc/2 + lead(csum, 1),
         pos = if_else(is.na(pos), perc/2, pos))
  

  
#ggplot(dfpartial_topic, aes(x="", y = perc, fill=categorie_sender_receiver)) + geom_col()+geom_text(aes(label=labels), position = position_stack(vjust = 0.5))+
